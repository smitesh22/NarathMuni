...
const googleAuthHandler = passport.authenticate("google", { scope: ["profile", "email"] }) as express.RequestHandler;
...
router.get(
    ...
    async (req: Request, res: Response): Promise<void> => {
        ...
        const { user, token } = req.user as { user: { id: string; email: string }; token: string };
        const { id, email } = user;
