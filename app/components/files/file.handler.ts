  ...
  try {
    switch (req.method) {
      ...
      case "GET":
        ...
        const contentObject = await contentObjectService.getContentObjectById(
        ...
        if (
          contentObject.extensions &&
          typeof contentObject.extensions === "object"
        ) {
          const filePath =
          ...
          if (filePath) {
            ...
            res.setHeader(
              "Content-Disposition",
              `attachment; filename="${decodeURI(filename)}"`,
            );
